...
function updateControls(){
 for(const id of ['angle','blur','padding','radius','shadow']){const el=$(id),unit=id==='angle'?'°':['blur','radius'].includes(id)?' px':'%';$(id+'-value').value=el.value+unit;el.parentElement.style.setProperty('--fill',((+el.value-+el.min)/(+el.max-+el.min)*100)+'%')}
 ...
 $('upload-name').textContent=customName||'No image uploaded. PNG, JPEG or WebP, up to 20 MB.';
 $('upload').textContent=customImage?'↑  Replace image':'↑  Upload image';
}
...
async function render(token){
 ...
 if(o.background!=='none'||o.padding||o.radius||o.shadow){const c=KakomiCompose.render(sourceImage,o,customImage);w=c.width;h=c.height;blob=await new Promise((resolve,reject)=>c.toBlob(b=>b?resolve(b):reject(Error('Could not export this image. Reduce padding and try again.')),'image/png'))}
...
function selectTab(tab){selectedTab=tab;if(tab==='image'&&!customImage)$('remove-background').checked=true;else enableBackground();scheduleRender()}
...
async function init(){
 ...
 $('edit-controls').addEventListener('input',e=>{const id=e.target.id;if(id==='wallpaper-file')return;if(['solid-color','gradient-start','gradient-end','angle'].includes(id))enableBackground();scheduleRender()});
 ...
 $('wallpaper-file').addEventListener('change',async e=>{
  ...
  try{if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>20*1024*1024)throw Error('Choose a PNG, JPEG or WebP smaller than 20 MB.');const bitmap=await createImageBitmap(file);if(token!==fileRevision){bitmap.close();return}if(bitmap.width*bitmap.height>64000000){bitmap.close();throw Error('Choose a background smaller than 64 megapixels.')};customImage?.close();customImage=bitmap;customName=file.name;selectedTab='image';enableBackground()}
