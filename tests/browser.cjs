...
(async()=>{
 ...
 // Both destinations, then clipboard failure fallback.
 ...
 await settingsPage.addInitScript(()=>{globalThis.prefs={};globalThis.chrome={storage:{local:{get:async d=>({...d,...prefs}),set:async p=>{prefs=p}}}}});
 ...
 assert.equal(await settingsPage.locator('#copyClipboard').isChecked(),true);
 assert.equal(await settingsPage.locator('#openPreview').isChecked(),false);
 await settingsPage.locator('#copyClipboard').click();assert.equal(await settingsPage.locator('#copyClipboard').isChecked(),true);
 await settingsPage.locator('#openPreview').click();await settingsPage.waitForFunction(()=>prefs.openPreview===true);
 await settingsPage.locator('#copyClipboard').click();await settingsPage.waitForFunction(()=>prefs.copyClipboard===false);
 ...
 console.log('PASS: geometry, 2x PNG pixels, element selection, reverse drag, drag cancellation, thin-area rejection, clipboard default, both destinations, copy failure fallback, settings save and validation, overlay removal, click suppression, clipping, iframe, shadow DOM, cleanup, tab guard, preview, download, mobile layout.');
