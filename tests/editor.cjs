...
await page.addInitScript(() => {
 ...
 globalThis.nativeAttempts=0;globalThis.browser={storage:globalThis.chrome.storage,runtime:{getURL:()=> 'chrome-extension://editor/'},clipboard:{setImageData:async()=>{globalThis.nativeAttempts++;throw Error('Access to extension API denied.')}}};
 const NativeClipboardItem=globalThis.ClipboardItem;globalThis.ClipboardItem=function(data,options){globalThis.clipboardInput=data['image/png'];return new NativeClipboardItem(data,options)};globalThis.ClipboardItem.prototype=NativeClipboardItem.prototype;
...
const pixels=()=>page.evaluate(async()=>{const im=await createImageBitmap(await(await fetch(document.querySelector('#capture').src)).blob());const c=document.createElement('canvas');c.width=im.width;c.height=im.height;const x=c.getContext('2d');x.drawImage(im,0,0);return{w:c.width,h:c.height,corner:[...x.getImageData(0,0,1,1).data],opposite:[...x.getImageData(c.width-1,c.height-1,1,1).data],edge:[...x.getImageData(48,48,1,1).data],center:[...x.getImageData(200,120,1,1).data]}});
await ready();const original=await bytes(),initial=await pixels();assert.equal(initial.w,496);assert.equal(initial.h,336);assert.equal(await page.locator('#padding').inputValue(),'20');assert.equal(await page.getByRole('tab',{name:'Gradient',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.locator('#remove-background').isChecked(),false);assert.equal(initial.corner[3],255);assert.equal(initial.opposite[3],255);assert.ok(initial.corner[0]<initial.opposite[0]);assert.ok(initial.corner[2]>initial.opposite[2]);
// A completed styled render must enter the browser clipboard as a concrete Blob.
await page.locator('#copy').click();await page.waitForFunction(()=>copyCount===1||document.querySelector('#notice').textContent.startsWith('Copy failed.'));assert.equal(await page.evaluate(()=>nativeAttempts),0);assert.equal(await page.locator('#notice').textContent(),'Copied current screenshot.');assert.equal(await page.evaluate(()=>clipboardInput instanceof Blob),true);assert.deepEqual(await page.evaluate(async()=>Array.from(new Uint8Array(await copied.arrayBuffer()))),original);assert.deepEqual(await page.evaluate(async()=>{const im=await createImageBitmap(copied);const c=document.createElement('canvas');c.width=im.width;c.height=im.height;const x=c.getContext('2d');x.drawImage(im,0,0);return{w:c.width,h:c.height,corner:[...x.getImageData(0,0,1,1).data],opposite:[...x.getImageData(c.width-1,c.height-1,1,1).data]} }),{w:initial.w,h:initial.h,corner:initial.corner,opposite:initial.opposite});await page.evaluate(()=>{copyCount=0;copied=null;clipboardInput=null});
await page.getByRole('tab',{name:'Image',exact:true}).click();await ready();await slide('radius',32);assert.equal((await pixels()).corner[3],0);
...
assert.equal(await page.evaluate(()=>clipboardInput instanceof Promise),true);
...
await page.evaluate(()=>{kakomiAPI.runtime.getURL=()=> 'moz-extension://editor/';kakomiAPI.clipboard={setImageData:async data=>{globalThis.nativeCopy=Array.from(new Uint8Array(data))}}});await page.locator('#copy').click();await page.waitForFunction(()=>globalThis.nativeCopy);assert.deepEqual(await page.evaluate(()=>nativeCopy),await bytes());
// Background reset restores Dawn without changing frame controls; frame reset keeps the selected background.
await page.locator('[data-reset="background"]').click();await ready();assert.equal(await page.getByRole('tab',{name:'Gradient',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.locator('#remove-background').isChecked(),false);assert.equal(await page.locator('#padding').inputValue(),'45');assert.equal((await pixels()).corner[3],255);
await page.getByRole('tab',{name:'Color',exact:true}).click();await page.getByRole('button',{name:'Rose',exact:true}).click();await ready();await page.locator('[data-reset="frame"]').click();await ready();assert.equal(await page.getByRole('tab',{name:'Color',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await page.locator('#solid-color').inputValue(),'#f4cbd5');assert.equal(await page.locator('#padding').inputValue(),'20');
